...
### Requirement: Daemon auto-shutdown on session count
**Reason**: Idle time is the right signal; counting active
sessions races with transient disconnects and would surprise
users who briefly close all their files.
**Migration**: If a future need arises for session-count-based
shutdown, add it as a new capability rather than reviving this
requirement.
