...
### Requirement: Bridge auto-installs a daemon
**Reason**: The daemon is owned by `pike-lsp` itself via
auto-start on first connection. The bridge only owns the LSP
forwarder.
**Migration**: If a future need arises for the bridge to manage
the daemon lifecycle, add it as a new capability rather than
reviving this requirement.
