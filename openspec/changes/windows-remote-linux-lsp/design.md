...
Zed remote SSH keeps the UI on the local host but runs language servers, terminals, and tasks on the remote server. The bridge currently calls `worktree.which("pike-lsp")` first, which is correct because Zed scopes that lookup to the worktree. The failure path is the fallback: `zed::current_platform()` describes the local extension host, so on a Windows UI with a Linux SSH worktree the bridge can select the Windows release asset. The downloaded command is then not runnable in the Linux worktree. The current manifest also lacks `[language_servers.pike-lsp]`, so Zed has no manifest-level LSP registration.
...
- Choose auto-download assets for the worktree execution platform when the worktree clearly looks like Linux, even from a Windows host.
...
- Detect every possible remote OS. Zed remote servers currently support Linux and macOS, while this change targets the reported Windows-host/Linux-remote path.
...
1. **Infer target platform from worktree context before falling back to `current_platform()`.**
   - Rationale: `worktree.shell_env()` and `worktree.root_path()` are the only available API inputs that describe the worktree execution environment in `zed_extension_api` 0.6. A Linux remote worktree exposes Unix-style roots and shell variables such as `HOME=/home/...`, `SHELL=/bin/...`, and colon-separated `PATH`.
...
- **Remote macOS from Windows is not fully inferred** → Current user need is Linux remote; unknown Unix defaults to Linux only when the root/env strongly indicates Linux. macOS can still work when `pike-lsp` is on remote PATH.
...
- Linux SSH remote worktrees will first use remote PATH and otherwise request the Linux release asset.
