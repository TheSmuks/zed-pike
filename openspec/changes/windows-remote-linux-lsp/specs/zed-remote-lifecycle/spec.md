...
The extension SHALL rely on Zed's worktree process-launch semantics for SSH remoting. When Zed is connected to a remote SSH worktree, `language_server_command` SHALL return a command that Zed executes in that remote context, not a locally initiated SSH tunnel. If the bridge must auto-download `pike-lsp` for a Linux SSH remote worktree opened from a Windows host, it SHALL select a Linux command path and Linux release asset rather than a local Windows executable.
...
#### Scenario: Windows host remote fallback is Linux runnable
- **WHEN** Zed on Windows opens a Pike file from a Linux SSH remote worktree
