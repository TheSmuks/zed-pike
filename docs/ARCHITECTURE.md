...
3. Fresh auto-download from the latest `TheSmuks/zed-pike` GitHub
   release, selected for the worktree execution platform. Any local UI
   host connected to a Linux SSH worktree selects the Linux asset; a
   local Windows worktree selects the Windows `.zip` and `pike-lsp.exe`.
...
When the worktree is Linux, fallback auto-download uses the Linux release
asset and the Unix `pike-lsp` filename regardless of local UI host OS. For
local Windows worktrees it uses the Windows `.zip` and `pike-lsp.exe`. The
bridge must not:
