...
### Windows host with Linux SSH remote

Use Zed's built-in SSH remote workflow. The extension does not start `ssh` itself:

1. Connect with Zed to the Linux remote workspace.
2. Open a Pike file from that remote worktree.
3. The bridge first checks the remote worktree PATH for `pike-lsp`.
4. If `pike-lsp` is not on PATH, the fallback selects the Linux release asset (`x86_64-unknown-linux-gnu.tar.gz`) even when the Zed UI is running on Windows.

For local Windows worktrees, the fallback selects the Windows release asset (`x86_64-pc-windows-msvc.zip`) and runs `pike-lsp.exe`.
